...
const JoinTeamsConfig: React.FC<Props> = ({ containerVisibleWidget, setContainerVisibleWidget }) => {
  const [showSettingsTeam, setShowSettingsTeam] = useState<boolean | null>(null);

  useEffect(() => {
    const fecthData = async () => {
      const data = await fetch("https://localhost:5187/api/Team/showSettings", {
                method: "GET",
                headers: {
                    "Content-type": "application/json"
                },
      ...
      setShowSettingsTeam(json.showSettings)
      return json.showSettings;
      ...
      <ButtonWidget
        onClick={() =>
          containerVisibleWidget === "teamSelection"
            ? setContainerVisibleWidget("")
            : setContainerVisibleWidget("teamSelection")
        }
        ...
        viewBox="0 0 24 24"
      />
      {/* {containerVisibleWidget === "teamSelection" ? <h1>muestra settings</h1> : null} */}
      {/* {showSettingsTeam === true ? <h1>se muestra los setings</h1>: <h2>tu ya tines equipoo</h2>} */}
        ...
        <div className="fixed border-2 border-gray-200 shadow-sm rounded-xl bg-white m-5 w-100 h-150 top-35 left-1">
          <div className="flex items-center justify-center bg-blue-700 w-auto rounded-xl h-10 text-white font-bold">
            ...
            <button onClick={() => setContainerVisibleWidget("")}>
              <svg
                className="absolute top-3 left-90 w-5 h-5 text-gray-800 dark:text-white cursor-pointer"
                aria-hidden="true"
          ...
          {showSettingsTeam === true ?
          <>
          <ButtonJoinTeam colorButton="bg-sky-500" onClick={async () => await joinTeam({ data: "1" })}>
                Unirte al equipo Sky
            </ButtonJoinTeam>
            <br />
            <ButtonJoinTeam colorButton="bg-stone-600" onClick={async () => await joinTeam({ data: "2" })}>
                Unirte al equipo Rock
            </ButtonJoinTeam>
            <br />
            <ButtonJoinTeam colorButton="bg-yellow-500" onClick={async () => await joinTeam({ data: "3" })}>
                Unirte al equipo Petroleros
            </ButtonJoinTeam>
            <br />
            <ButtonJoinTeam colorButton="bg-green-600" onClick={async () => await joinTeam({ data: "9" })}>
                Unirte al equipo Eco
            </ButtonJoinTeam>
          </>
            
          : <h2>tu ya tines equipoo</h2>}
        </div>
      ) : null}
    </>
  );
